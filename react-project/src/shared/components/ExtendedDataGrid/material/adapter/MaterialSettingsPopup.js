...
export default class MaterialSettingsPopup extends UIComponent {
  ...
  render() {
    return (
      <div style={{ width: 540 }}>
        <Typography style={{ margin: "15px", fontSize: 20 }}>
          Settings:
        </Typography>
        <div style={{ marginLeft: 15, width: 500 }}>
          <MaterialDataGrid
            dataProvider={this._cols}
            enableActiveCellHighlight={false}
            height={300}
            selectedObjects={
              this._cols.length === this._visibleCols.length
                ? this._cols
                : this._visibleCols
            }
            width={"100%"}
            onChange={(evt) => {
              this.selectedColumns = evt.grid.getSelectedObjects();
            }}
          >
        ...
        <div className={"options"}>
            ...
            <FormGroup>
              <FormControlLabel
                control={
                  <Checkbox
                    className={"cbFooter"}
                    defaultChecked={this._footerVisible}
                    style={this._enableFooters ? {} : { visibility: "hidden" }}
                    onChange={(evt, newValue) => {
                      this._footerVisible = newValue;
                    }}
                  />
                }
              ...
              <FormControlLabel
                control={
                  <Checkbox
                    className={"cbFilters"}
                    defaultChecked={this._filterVisible}
                    style={this._enableFilters ? {} : { visibility: "hidden" }}
                    onChange={(evt, newValue) => {
                      this._filterVisible = newValue;
                    }}
                  />
                }
          ...
          <div
            style={{
              marginLeft: "15px",
              display: this._enablePaging ? "" : "none",
            }}
          >
            <Typography>
              {Constants.SETTINGS_RECORDS_PER_PAGE + "  "}
            </Typography>
            <TextField
              className={"txtPageSize"}
              defaultValue={this._pageSize || 50}
              name="perPage"
              style={{ width: "auto" }}
              onChange={(evt) => {
                this._pageSize = parseInt(evt.currentTarget.value);
              }}
            />
          </div>
        </div>
 ...
 * @type {Function}
 */
MaterialSettingsPopup.prototype.typeName = MaterialSettingsPopup.typeName =
  "MaterialSettingsPopup"; //for quick inspection

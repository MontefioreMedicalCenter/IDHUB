...
class MaterialMultiSelectComboBoxCheckBox extends MaterialTristateCheckBox {
  ...
  determineCheckBox() {
    ...
    if (this.labelText) this.children.push(this.labelText);
  }
}
export default class MaterialMultiSelectComboBox extends MultiSelectComboBox {
  addTemplate() {
    const template = (
      <span>
        ...
        />
        <IconButton
          className={"insideIcon inputIcon"}
          style={{
            width: "40px",
            height: "40px",
            position: "absolute",
        ...
        <IconButton
          className={"outsideIcon inputIcon"}
          style={{
            display: "none",
            width: "4px",
            height: "4px",
    ...
    this.addEventListener(this, Constants.EVENT_CLICK, this.onClick);
    this._textBox = {};
  }
  getTextBox() {
  ...
  onCheckDelayedChange(e) {
    ...
    if (cbox.labelText === this.addAllItemText)
      this.allUnchecked =
        cbox.getSelectedState() === TriStateCheckBox.STATE_UNCHECKED;
    ...
    if (this.selectedValues.length === 0 && !wasAllUnchecked) {
      ...
      if (cbox.labelText === this.addAllItemText) {
        ...
        for (
          let i = this.getAddAllItem() ? 1 : 0;
          i < this.getDataProvider().length;
          i++
        ) {
      ...
      if (cbox.labelText === this.addAllItemText) {
        ...
        if (
          cbox.getSelectedState() === TriStateCheckBox.STATE_CHECKED &&
          this.selectedValues.indexOf(cbVal) === -1
        ) {
    ...
    const okB = UIUtils.findElementWithClassName(
    ...
    if (this.popup) {
      for (let i = 0; i < this.checkboxes.length; i++) {
        ...
        if (this.allUnchecked) {
          ...
          cb.setSelectedState(
            this.selectedValues.length >= this.getDataProvider().length - 1
              ? TriStateCheckBox.STATE_CHECKED
              : this.selectedValues.length > 0
              ? TriStateCheckBox.STATE_MIDDLE
          ...
          cb.setSelectedState(
            this.selectedValues.includes(cbVal)
              ? TriStateCheckBox.STATE_CHECKED
              : TriStateCheckBox.STATE_UNCHECKED,
          );
        }

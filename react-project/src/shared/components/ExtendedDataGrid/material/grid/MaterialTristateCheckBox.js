...
export default class MaterialTristateCheckBox extends TriStateCheckBox {
  ...
  determineCheckBox() {
    const cb = this.getMiddle() ? (
      ...
      this.getSelected() ? (
        <Checkbox key="cb" className={"materialCheckBox"} checked />
      ) : (
        <Checkbox key="cb" className={"materialCheckBox"} checked={false} />
      )
    ) : this.getSelected() ? (
      <Checkbox key="cb" checked disabled />
    ) : (
      <Checkbox key="cb" disabled checked={false} />
    );
